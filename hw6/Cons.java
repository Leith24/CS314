...
public static Cons vars (Object expr) {
	...
	else if(stringp(expr)&&assoc(expr,opprec)==null){
		return list(expr);
	}
	else return null;
}
...
public static boolean occurs(Object value, Object tree) {
	if(consp(tree)){
		return occurs(value, first((Cons)tree))||occurs(value,rest((Cons)tree));
	}
...
//	System.out.println(tree);
	if(consp(tree)){
		...
		else return pow(eval(lhs((Cons)tree)),eval(rhs((Cons)tree)));
	}
	else if(numberp(tree)){
		return (Integer) tree;
	}
...
public static Integer eval (Object tree, Cons bindings) {
	if(consp(tree)){
		...
		else return pow(eval(lhs((Cons)tree),bindings),eval(rhs((Cons)tree),bindings));
	}
...
public static Cons english (Object tree) {
	if(consp(tree)){
		if(op((Cons)tree)=="-"&&rhs((Cons)tree)==null){
			return cons("negative",english(lhs((Cons)tree)));
		}
		else if(op((Cons)tree)!="expt"){
		return cons("the",cons(lhs(assoc(op((Cons)tree),engwords)),cons("of",cons(english(lhs((Cons)tree)),cons("and",english(rhs((Cons)tree)))))));
		}
		else{
		return cons(english(lhs((Cons)tree)),cons("to the",cons(lhs(assoc(op((Cons)tree),engwords)),cons("of",english(rhs((Cons)tree))))));
		}
	}
	else if(numberp(tree)){
...
//	System.out.println(tree);
//	System.out.println(lhs(assoc(op((Cons)tree),opprec)));

	int newprec = 0;
	if(consp(tree)){
		if(assoc(op((Cons)tree),opprec)==null&&rhs((Cons)tree)==null){
			return "Math."+op((Cons)tree)+"("+tojavab(lhs((Cons)tree),prec)+tojavab(rhs((Cons)tree),prec)+")";
		}
		else if(op((Cons)tree)=="-"&&rhs((Cons)tree)==null){
			return "-"+tojavab(lhs((Cons)tree),prec);
		}
		else{
			if(op((Cons)tree)=="+"||op((Cons)tree)=="-"){
				newprec = 5;
			}
			else if (op((Cons)tree)=="*"||op((Cons)tree)=="/"){
				newprec =6;
			}
			if(newprec<=prec&&newprec!=0){
				return "("+tojavab(lhs((Cons)tree),newprec)+op((Cons)tree)+tojavab(rhs((Cons)tree),newprec)+")";
			}
			else{
				return tojavab(lhs((Cons)tree),newprec)+op((Cons)tree)+tojavab(rhs((Cons)tree),newprec);	
			}
