...
public int astar( Vertex start, Vertex goal, Heuristic h ) {
  int rm = 0;
  boolean done = false;
//  System.out.println("start: "+start.str()+"  goal:  "+goal.str());
  //	System.out.println(s.str());
  int cost = 0;
  for ( Vertex v : vertices.values()  ) {
    v.visited = false;
...
public static final Heuristic halfass = new Heuristic() {
	public Integer fn(Vertex from, Vertex to) {
		return (Integer)(int)((from.distanceTo(to))*.5);}
 };
...
public Cons pathto(String city) {
	Cons list = null;
	String s =(vertices.get(city).str());
	Vertex j = vertices.get(city);
	//System.out.println(s);
	list = list.cons(s,list);
	for( Vertex v : vertices.values()){
		if(j.parent != null){
			v = j.parent();
			String i = v.str();
