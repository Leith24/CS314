...
// empty buffers into a music string
	...
	if( lhs(e).equals(v)) return e;
		...
		for(; lst != null; lst = rest(lst)){
			//System.out.println("lst :   "+first(lst));
			Cons e1 = (Cons)transform((Cons)first(lst),e);
			//System.out.println("e1:  "+e1);
			if(e1 != null&&(!(numberp(rhs(e1))))){
				if(solve(e1,v)!=null){
					return solve(e1,v);
				}	
			}
	...
	else if(!(rhs(e).equals(v))&&(!(consp(rhs(e))))) return null;
	return null;
    }
    ...
    public static int conshash(Object e) {
    	...
    	int hash1 = 0;
    	int hash2 = 0;
    	if((Cons)e==null) return 0;
	else if(rest((Cons)e)==null){
		hash1 += (first((Cons)e).hashCode())*17;
		hash ^= hash1;
		return hash;
	}
    	else{
    		//System.out.println(first((Cons)e));
    		hash1 += (first((Cons)e).hashCode())*17;
    		hash2 += (rest((Cons)e).hashCode())*127;
    		hash ^= hash1;
    		hash ^= hash2;
    		return hash;
    	}
    ...
    public static int totaltime(Cons action) {
    	    int time = 0;
  	    /*for(int i = 0; i<insttime.length;i++){
    	    	time += insttime[i];	    
    	    }*/
    	    if(first(action).equals("repeat")){
    	    	time = (int)(Integer)second(action)*totaltime((Cons)third(action));		    
    	    }
    	    else if(first(action).equals("seq")){
    	    	for(Cons lst = rest(action); lst != null; lst = rest(lst)){
    	    	       Cons e = (Cons)first(lst);
    	    	       time+=totaltime(e);
        	}	    
    	    }
    	    	...
    	    	for(Cons lst = rest(action); lst != null; lst = rest(lst)){
    	    	       Cons e = (Cons)first(lst);
    	    	       max = Math.max(totaltime(e),max);
        	}	  		    
        	time = max;
    	    }
    	    else if(first(action).equals("piano")){
    	    	time =	(int)(Integer)fourth(action);	    
    	    }
    	    else{
    	    	time= (int)(Integer)second(action);	    
    	    }
    	    return time;
}
	...
	if(act==null) return;
        ...
        if ( command.equals("boom") ) {
        	...
        	emit(4,time+(((int)(Integer)second(act))/4),((int)(Integer)second(act))/4);
        	emit(4,time+(((int)(Integer)second(act))/2),((int)(Integer)second(act))/2);
        	return;
        }
        else if(command.equals("tambourine")){
        	emit(11,time,2);
        	double r = random.nextDouble();
        	int ra = (int)(((int)(Integer)second(act)*r));
        	if(ra>=2){
        		time += ((int)(Integer)second(act))-ra;
        		addevent(pq,list("tambourine",ra),time);
        		return;
        	}
        	else{
        		return;
        	...
        	emitp((int)(Integer)second(act),(String)third(act),insttime[(int)(Integer)second(act)],(int)(Integer)fourth(act));
        	return;
        }
        else if(command.equals("seq")){
        	int tm = 0;
        	for(Cons lst = rest(act); lst != null; lst = rest(lst)){
        		Cons e = (Cons)first(lst);
        		addevent(pq,e,time+tm);
        		tm+=totaltime(e);
        	}
		return;		
        }
        else if(command.equals("repeat")){
        	int repeat = (int)(Integer)second(act);
    ...
    public static Cons round( Cons melody ) {
    	    ...
    	    int delay1 = (totaltime(melody))/4;
    	    int delay2 = (totaltime(melody))/2;
    	    return list("sync",voice1,list("seq",list("rest",delay1),voice2),list("seq",list("rest",delay2),voice3));
    }
