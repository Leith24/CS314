...
//	System.out.println(tree);
	if(consp(tree)){
	//	System.out.println("op:  "+op((Cons)tree));
...
public static Double solveqns(Cons eqns, Cons vals, String v) {
	if(eqns==null||vals==null) return null;
	if(lhs(assoc(v,vals))!=null){
		return (Double)(lhs(assoc(v,vals)));
	}
	else{

//		System.out.println(vars(vals));
		Cons variables = vars((Cons)first(eqns));
		Cons vars = vars(vals);
		Cons newvals = helper(variables,vals, eqns);
		return solveqns(eqns,newvals,v);
	}
}
...
public static Cons helper(Cons variables, Cons vals, Cons eqns){
	if(variables==null||eqns==null) return vals;
	variables = vars((Cons)first(eqns));
/*	String v = solver(variables, vals);
	if(v != null){
		Double d = eval(solve((Cons)first(eqns),v),vals);
		return cons(list(v,d),vals);
}
else{*/
	if(lhs(assoc(first(variables),vals))==null){
		Double d = eval((Cons)rhs((Cons)first(eqns)),vals);
		return cons(list(first(variables),d),vals);
		
	}
	else{
		return helper(rest(variables),vals,rest(eqns));	
	}

}
public static String solver(Cons variables, Cons vals){
	if(variables==null) return null;
	if(lhs(assoc(first(variables),vals))!=null){
		return solver(rest(variables),vals);	
	}
	else{
		return (String)first(variables);	
	}
}
    ...
    public static Cons substitutions = readlist( list(
       ...
       "( (?function mintree) (?comibine min) (?baseanswer (if (numberp tree) tree 999999)))",
	...
	"( (deriv (sin (* ?y ?x)) ?x) (* (cos (* ?y ?x)) (deriv (* ?y ?x) ?x)))",
	"( (deriv (sin ?x) ?x)   (* (cos ?x) (deriv ?x ?x)))",
	"( (deriv (sqrt ?u) ?x)  (* (/ 1 2) (* (expt ?u (/ (- 1) 2)) (deriv ?u ?x)))))",

	"( (deriv (log (expt (?x) ?y)) ?x) (/ (deriv(expt ?x ?y)) (expt ?x ?y)))",
 ...
 // add more
	...
	"( (not ?x) (zlparen ! zlparen ?x zrparen zrparen))",
	"( (<= ?x ?y) (zlparen ?x zspace <= zspace ?y zrparen))",
	"( (return ?x) (zreturn ztab return zspace ?x ; zreturn))",
	"( (if ?test ?then ?else) (if zspace zlparen zspace ?test zspace zrparen ?then zreturn else zspace ?else))",
	"( (if ?test ?then ?elseif ?else) (if zspace zlparen zspace ?test zspace zrparen ztab ?then else if zspace zlparen ?esleif zrparen else zspace ?else))",
	"( (+ ?x ?y) ( zlparen ?x zrparen zspace + zspace zlparen ?y zrparen))",
	"( (add1 ?x ?y) (add1 zlparen ?x , zspace ?y zrparen))",
	"( (?combine ?x ?y) (Math.min zlparen ?x , zspace ?y zrparen))",
       // add more
    ...
    public static void main( String[] args ) {
        ...
        System.out.println("magnification = " +
              ...
              reader("((subject_distance 6.0)(focal_length 9.0)(radius 4.5)(image_distance -18.0))"),
                                    "magnification"));
        ...
        System.out.println("power = " +
             ...
             reader("((weight 700.0)(height 8.0)(time 10.0)(mass 71.3801349)(force 700.0)(speed .8)(power 560.0))"),
                                    "power"));
