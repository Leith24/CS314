...
throws java.io.FileNotFoundException
 {
    ...
    // ****** your code starts here ******
        ...
        final Mapper map2 = new Mapper() {
            ...
            //	for(char c = 'a'; c <= 'z';c++){
            	...
            	for(int i = 0; i<value.length();i++){
            		if((value.charAt(i)==c||value.charAt(i)==d)&&i+1<=value.length()){
            		mr.collect_map(value.substring(i,i+1).toLowerCase(), list("1"));
            		}
            	}
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() { //close
            public void map(String key, String value, MapReduce mr) {
            	...
            	while (st.hasMoreTokens()){
            		...
            		//System.out.println(s);
            			mr.collect_map(s.toLowerCase(),list("1"));
            	}
        ...
        final Mapper map4 = new Mapper() {
           ...
           // 	System.out.println(key);
                ...
                while (st.hasMoreTokens()){
            		String s = st.nextToken();
            		//System.out.println(s);
            		if(s.contains(grepkey))
            			mr.collect_map(grepkey.toLowerCase(),list(key));
            	}
            } };
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	Cons val = null;
            	for (Cons lst = value; lst != null; lst = rest(lst))
            		val = cons(first((Cons)first(lst)),val);
            	mr.collect_reduce(key, nreverse(val));
            } };
        ...
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	while (st.hasMoreTokens()){
            		String s = st.nextToken();
            			mr.collect_map(s.toLowerCase(),list(key));
            	}
            } };
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	while (st.hasMoreTokens()){
            		...
            		rating = st.nextToken();
            			mr.collect_map(s.toLowerCase(),list(rating));
            		break;
            	}
            } };
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	double rating = 0;
            	for (Cons lst = value; lst != null; lst = rest(lst)){
            		rating += Integer.decode((String) first((Cons)first(lst)));
            		sum ++;
            }
            	key = key + " "+ (rating/(double)sum);
